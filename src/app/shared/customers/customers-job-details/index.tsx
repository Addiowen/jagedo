...
export default function ActiveJobDetailsCard({transactionDetails}:{transactionDetails:any}) {
  ...
  const professionalActiveJobDetailsData = 
    {
      ...
      Status: transactionDetails.statusHistory.length > 0 ? transactionDetails.statusHistory[0].status : "",
      'Start Date': transactionDetails.startDate ? new Date(transactionDetails.startDate).toISOString().split('T')[0] : "",
      'End Date': transactionDetails.endDate ? new Date(transactionDetails.endDate).toISOString().split('T')[0] : "",
      'Invoice Number': transactionDetails.id || "",
      'Payment Status': transactionDetails.statusHistory.length > 1 ? transactionDetails.statusHistory[1].status : "",
      Amount: transactionDetails.metadata.amount ? transactionDetails.metadata.amount.toString() : ""
    }
  ;
      ...
      <div className='mb-4'>
        {
            ...
            <ChunkedGrid data={ professionalActiveJobDetailsData} requestDetails={ professionalActiveJobDetailsData}dataChunkSize={8} className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 gap-6' />
        }
      </div>
    </>
  );
