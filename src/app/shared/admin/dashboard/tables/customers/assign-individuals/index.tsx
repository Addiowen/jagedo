...
export default function AssignIndividuals({
  className,
  customers,
}: {
  className?: string;
  ...
  const indiviualCustomers = customers.results.filter(
  ...
  console.log(indiviualCustomers);

  const filteredCustomers =
    indiviualCustomers?.map((item: any, index: number) => {
      return {
        no: index + 1,
        id: item.id || '',
