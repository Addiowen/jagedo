...
import {
  ACTIVE_JOBS_DEFAULT_VALUE,
  activeJobsSchema,
  ActiveJobsType,
  COMPLETE_JOBS_DEFAULT_VALUE,
  completeJobsSchema,
} from '@/utils/active-jobs.schema';
...
export default function CompleteJobDetailsAttachments() {
  ...
  return (
    <form className="mb-4 @container">
      {/* <div> */}
      <div className="rounded-lg border border-muted bg-gray-0 px-2 pb-12 pt-6 dark:bg-gray-50 sm:rounded-sm lg:rounded-xl xl:rounded-2xl">
                ...
                <Fragment key={`attachments-table-${index}`}>
                  <SortableList.Item id={field.id}>
                    <div className="group grid min-h-10 grid-cols-6 gap-0 border-b border-muted dark:border-muted/20">
                      <div className="col-span-1 ms-4 flex items-center justify-center p-2 pb-4">
                        <Text className="text-center font-semibold text-gray-500">
                          {index + 1}
                        </Text>
                      </div>

                      <div className="col-span-1 flex items-center justify-center p-2 pb-4">
                        <Text className="text-center text-gray-900 dark:text-gray-0">
                          Milestone {index + 1}
                        </Text>
                      </div>

                      <div className="col-span-2 flex items-center justify-center p-2 pb-4">
                        {field.docName ? (
                          <div className="">
                            <Text className="flex text-center text-gray-900 dark:text-gray-0">
                              {field.docName}
                            </Text>
                          </div>
                        ) : (
