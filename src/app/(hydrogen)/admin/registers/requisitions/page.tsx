...
const fetchTransactions = async () => {
  try {
    const transactionDetails = await apiRequest({
      method: 'GET',
      endpoint: `/transactions?orderBy=createdDate&order=asc`,
    });
...
export default async function AdminRequisitionsPage() {
  ...
  const formattedData = transactions?.results.map((item: any, index: number) => {
    ...
    return {
      ...
      requestType: `${item.metadata?.packageType} : managed by ${item.metadata?.managed}` || "",
  ...
  return (
    <div className="@container">
      ...
      <div className="grid grid-cols-1  @4xl:grid-cols-2  3xl:gap-8">
        <AdminRequisitionsTable requests={formattedData} className="relative  @4xl:col-span-2 " />
      </div>
    </div>
  );
}
