...
export default function QuotationDropDownComponent() {
  const router = useRouter();

  const handleRedirect = (requestType: any) => {
    router.push(`${routes.admin.professionalQuotation}?type=${requestType}`);
  };
  return (
    <Dropdown>
      <Dropdown.Trigger>
        <Button as="span">
          ...
          New Quotation <BsChevronDown size="lg" className="ml-2 w-5" />
        </Button>
      </Dropdown.Trigger>
      <Dropdown.Menu>
